...
import pymysql # used to connect to mysql
...
if len(sys.argv) > 1: #sys.argv is the list of command-line arguments
...
# The method listdir() returns a list containing the names of the entries 
...
for filename in sorted(os.listdir(directory)): 
    ...
    with open(directory + '/' + filename) as json_data: # looks at files in file timestamp order
        ...
        # each station goes line by line over the data read
        for each_station in data_from_file:
            with connection.cursor() as cursor:
                ...
                result = cursor.fetchone() # method retrieves the next row of a query result
